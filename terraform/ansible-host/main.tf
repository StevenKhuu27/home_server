...
# Stage the cloud-init user-data snippet on the Proxmox host so the VM can
# reference it via `cicustom` below. Proxmox reads snippets from
...
resource "null_resource" "cloud_init_config_files" {
  ...
  provisioner "file" {
    source      = "${path.module}/user_data.yml"
